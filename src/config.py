...
class Config:
    ...
    def validate(self) -> None:
        ...
        if not self.corpus_type:
            raise ValueError("Corpus type is required (basch_narrative or basch_instructive)")
            
        if self.corpus_type not in ['basch_narrative', 'basch_instructive']:
            raise ValueError(f"Invalid corpus type: {self.corpus_type} (must be 'basch_narrative' or 'basch_instructive')")
